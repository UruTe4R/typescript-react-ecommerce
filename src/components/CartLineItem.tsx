...
const CartLineItem = ({ item, dispatch, REDUCER_ACTIONS }: PropsType) => {
  ...
  const optionValues: number[] = [...Array(highestQty).keys()].map(x => x + 1)
  ...
  //aria-label attribute is part of the Accessible Rich Internet Applications (ARIA) specification
  const content = (
    <li className="cart__item">
      ...
      //when we hover over the button, we see the title
...
//memo prevents component from re-rendering if the props haven't changed
//could give custom comparison function like below

function areItemsEqual({ item: prevItem }: PropsType, { item: nextItem }: PropsType) {
  return Object.keys(prevItem).every(key => {

    //return true if key is in both objects
